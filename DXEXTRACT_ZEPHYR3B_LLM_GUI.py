...
# ===== CONFIGURATION =====
# Set the device (Gradio runs on the server)
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# Update this path to your Zephyr model folder
MODEL_PATH = './zephyr_local_model'
...
loading_error = None # To store error message if loading fails

# ===== LOAD MODEL & DATA =====
def load_components():
    """Loads the tokenizer, model, and knowledge base."""
    global tokenizer, model, knowledge_base, actual_column_names, loading_error
    if tokenizer is not None: # Check if components are already loaded
        return

    try:
        # Loading tokenizer and model
        # If using LoRA, you'll need to load the base model first, then the adapters
        # Example for LoRA (requires peft installed):
        # from peft import PeftModel, PeftConfig
        # config = PeftConfig.from_pretrained(MODEL_PATH)
        # base_model = AutoModelForCausalLM.from_pretrained(config.base_model_name_or_path, torch_dtype=torch.float16, load_in_4bit=True if torch.cuda.is_available() else False, device_map="auto" if torch.cuda.is_available() else None)
        # model = PeftModel.from_pretrained(base_model, MODEL_PATH).to(DEVICE)
        # tokenizer = AutoTokenizer.from_pretrained(config.base_model_name_or_path)

        # If not using LoRA (standard model loading):
        ...
        # print(f"DEBUG: Actual column names from CSV: {actual_column_names}") # Removed print for GUI
        ...
        loading_error = f"Initialization failed: {str(e)}"
        print(loading_error) # Print error to console for debugging


# ===== USE LLM TO PARSE QUERY =====
def analyze_query(message, tokenizer, model):
    """Analyzes the user query using the LLM to predict intent and parameters."""
    system_prompt = (
    ...
    for json_str in json_matches:
        try:
            ...
            # print(f"DEBUG: Error parsing potential JSON: {e} in string: {json_str}") # Removed print for GUI
            continue
    ...
    # print(f"DEBUG: Parsed prediction: {parsed}") # Removed print for GUI
...
def generate_response(message, tokenizer, model, knowledge_base, actual_column_names):
    """Generates a response based on the user query and knowledge base."""
    if not message or not message.strip():
        return "⚠️ Please enter a valid question."

    prediction = analyze_query(message, tokenizer, model)
    # print("DEBUG Zephyr Prediction:", json.dumps(prediction, indent=2)) # Removed print for GUI
    ...
    if intent == 'lookup':
        ...
        for actual_col in actual_column_names:
            ...
            if actual_col in message_lower:
                identified_column_from_message = actual_col
                best_col_score_message = 100
                break
        ...
        for extract_name_key in knowledge_base.keys():
             ...
             if extract_name_key in message_lower:
        ...
        # print(f"DEBUG: Identified column from message: '{identified_column_from_message}' with score {best_col_score_message}") # Removed print for GUI
        # print(f"DEBUG: Identified identifier from message: '{identified_identifier_from_message}' with score {best_id_score_message}") # Removed print for GUI
        ...
        if any(phrase in message_lower for phrase in ["tell me about", "details for", "show me", "everything on", "details"]) or message_lower.strip().startswith("search for"):
             requesting_all_details = True
             # print("DEBUG: User is requesting all details.") # Removed print for GUI
        ...
        if value:
             ...
             if column and column in actual_column_names:
                  ...
                  # print(f"DEBUG: Strategy 1: Using LLM predicted column '{search_column}' for value search.") # Removed print for GUI
             # Otherwise, if a column was identified from the message with high confidence, use that
                  ...
                  # print(f"DEBUG: Strategy 1: Using message identified column '{search_column}' for value search.") # Removed print for GUI
             else:
                  # print("DEBUG: Strategy 1: No specific column determined for value search, searching all.") # Removed print for GUI
                  pass # Keep searching all columns if no specific column is determined
        ...
        # Strategy 2: If no value from LLM, but identifier and column are present, and identifier looks like a value
             ...
             # print(f"DEBUG: Strategy 2: Applying heuristic - Using identifier '{identifier}' as search value.") # Removed print for GUI
             ...
             if column and column in actual_column_names:
                  search_column = column
                  # print(f"DEBUG: Strategy 2: Using LLM predicted column '{search_column}' for value search.") # Removed print for GUI
             elif identified_column_from_message and best_col_score_message > 80:
                  search_column = identified_column_from_message
                  # print(f"DEBUG: Strategy 2: Using message identified column '{search_column}' for value search.") # Removed print for GUI
             else:
                  # print("DEBUG: Strategy 2: No specific column determined for value search, searching all.") # Removed print for GUI
                  pass # Keep searching all columns if no specific column is determined
        ...
        if not search_value and (identifier or identified_identifier_from_message):
             ...
             if identified_identifier_from_message and best_id_score_message > 70: # Use message identifier if identified with decent score
                  ...
                  # print(f"DEBUG: Strategy 3: Using message identified identifier: '{search_identifier}'") # Removed print for GUI
             # Otherwise, use LLM's predicted identifier if it exists
             elif identifier:
                  search_identifier = identifier
                  # print(f"DEBUG: Strategy 3: Using LLM predicted identifier: '{search_identifier}'") # Removed print for GUI
             ...
             if search_identifier:
                  ...
                  if column and column in actual_column_names:
                       search_column = column
                       # print(f"DEBUG: Strategy 3: User requested specific column '{search_column}' (LLM predicted).") # Removed print for GUI
                  # Otherwise, check if a column was identified from the message with high confidence
                  elif identified_column_from_message and best_col_score_message > 80:
                       search_column = identified_column_from_message
                       # print(f"DEBUG: Strategy 3: User requested specific column '{search_column}' (message identified).") # Removed print for GUI
        ...
        if search_value:
            ...
            # print(f"DEBUG: Performing value search for '{search_value_lower}' in column '{search_column}' (if specified)") # Removed print for GUI
            ...
            if search_column and search_column in knowledge_base.get(list(knowledge_base.keys())[0], {}):
                 ...
                 # print("DEBUG: No specific search column determined, searching all columns.") # Removed print for GUI
                 for key, data in knowledge_base.items():
            ...
            if matches:
                ...
                # print(f"DEBUG: No matches found for value '{search_value}'.") # Removed print for GUI
                if search_column:
            ...
            # print(f"DEBUG: Performing identifier search for '{search_identifier}'") # Removed print for GUI
            ...
            if fuzz.partial_ratio(best_match, identifier) > 65: # Slightly increased identifier threshold
                ...
                # print(f"DEBUG: Found extract '{best_match}' with score {fuzz.partial_ratio(best_match, identifier)}") # Removed print for GUI

                # If a specific column was requested (either by LLM or message)
                if search_column and search_column in extract_data:
                     # print(f"DEBUG: Returning specific column '{search_column}' for identifier '{best_match}'.") # Removed print for GUI
                    ...
                    # print(f"DEBUG: Returning all details for identifier '{best_match}'.") # Removed print for GUI
                    details = "\n".join([f"{k.title()}: {v}" for k, v in extract_data.items()])
                    return f"Details for {best_match.title()}:\n{details}"
            else:
                 # print(f"DEBUG: No good extract match found for identifier '{identifier}'. Best score: {fuzz.partial_ratio(best_match, identifier)}") # Removed print for GUI
...
# ===== GRADIO APP =====
# Load components once when the script starts
load_components()

def chatbot_response(user_input):
    """Function to generate bot response for Gradio interface."""
    global tokenizer, model, knowledge_base, actual_column_names, loading_error
    ...
    # Generate response using the core logic
...
iface = gr.Interface(
    ...
    outputs=gr.Textbox(), # Use Textbox for plain text output
    title="Data Extract Bot",
    description="Ask questions about the data extracts based on Source Code, Table Name, or Extract Name."
)

# Launch the Gradio interface
if __name__ == "__main__":
    # Check if components loaded before launching
    if loading_error:
        print("Gradio interface will not start due to component loading error.")
    else:
